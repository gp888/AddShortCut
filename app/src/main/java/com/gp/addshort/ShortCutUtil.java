...
public class ShortCutUtil {
    ...
    /** 增加一键清理快捷方式 */
    private static void addOneKeyCleanShortCut(Context context, Intent intent, boolean allowRepeat) {
        ShortcutIconResource iconRes = Intent.ShortcutIconResource.fromContext(
                context, R.drawable.weixin);
    ...
    private static Intent getShortCutIntent(Context context, int type) {
        ...
        switch (type) {
            case SHORTCUT_TYPE_OKC:
                intent.putExtra("from", "oneKeyCleanShortCut");
                intent.setClassName(context, SecondActivity.class.getName());
                break;
			 ...
			 */
                ...
                intent.setPackage("com.gp.game");
                intent.putExtra("from", "myGameShortCut");
     ...
     * @return 返回true为正常执行完毕，返回false为pkg值为null或者找不到该应用或者该应用无用于Launch的MainActivity 。
     * */
     ...
     * @param icon 快捷方式图标
     * @param allowRepeat 是否允许重复生成
     */
    public static void addShortcut(Context context, String shortcutName, Intent actionIntent, ShortcutIconResource icon, boolean allowRepeat) {
     ...
     * 更多请参考{@link #isShortCutExist(Context, String)}
